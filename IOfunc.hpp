...
// function to load a fasta file
template<typename uint_s>
void load_fasta_conc(const char *filename, std::vector<uint8_t>& Text, uint_s& sum, uint_s& ns){
    // open the input stream
    std::ifstream input(filename);
    if(!input.good()) { // exit if we cannot open the stream
        std::cerr << "Error opening " << filename << ". exiting..." << std::endl;
...
// function to load a fastq file
template<typename uint_s>
void load_fastq_conc(const char *filename, std::vector<uint8_t>& Text, uint_s& sum, uint_s& ns){
    // open the input stream
    std::ifstream input(filename);
    if(!input.good()) { // exit if we cannot open the stream
        std::cerr << "Error opening " << filename << ". exiting..." << std::endl;
...
template<typename uint_s>
void load_text(const char *filename, std::vector<uint8_t>& Text, uint_s& size){
    // open the input stream
    std::ifstream input(filename);
    if(!input.good()) { // exit if we cannot open the stream
        std::cerr << "Error opening " << filename << ". exiting..." << std::endl;
